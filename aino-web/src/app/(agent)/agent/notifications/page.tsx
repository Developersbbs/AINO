    ...
    <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            {unreadCount > 0 && <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>{unreadCount} unread</p>}
          </div>
        </div>
        {unreadCount > 0 && (
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
        ...
        {!isLoading && notifications.map((n, i) => (
          <div
            key={n.id}
            style={{ display: 'flex', gap: 14, padding: '14px 20px', borderBottom: i < notifications.length - 1 ? '1px solid #f1f5f9' : 'none', background: n.is_read ? 'white' : '#fafbff', cursor: n.is_read ? 'default' : 'pointer' }}
